...
bool CGameServerDlg::LoadQuestHelperTable()
{
	...
	pData->nIndex              = 1; // m_nQuestHelperID (must be > 0)
	pData->bMessageType        = 2; // ?
	pData->bLevel              = 1;
	pData->nExp                = 0;
	...
	pData->sEventDataIndex     = 0; // 30? // checked before running (sEventDataIndex == 0; depth of quest? - 0 is entry point?)
	pData->bEventStatus        = 0; // checked before running

	pData->nEventTriggerIndex  = 35001; // nEventID
	...
	pData = new _QUEST_HELPER;

	pData->nIndex              = 2; // m_nQuestHelperID (must be > 0)
	pData->bMessageType        = 2;
	pData->bLevel              = 1;
	pData->nExp                = 0;
	...
	// index into m_QuestMonsterArray, but is also the index into m_questMap
	pData->sEventDataIndex     = 30; // checked before running (sEventDataIndex == 0; depth of quest? - 0 is entry point?)
	pData->bEventStatus        = 0; // checked before running (the start of event 30)

	// start of event 30 so set nEventTriggerIndex to 35023 because that is the entry point (bEventStatus == 0)
	pData->nEventTriggerIndex  = 35023; // nEventID
	pData->nEventCompleteIndex = 0;
	pData->nExchangeIndex      = 0;
	pData->nEventTalkIndex     = 0;
	pData->strLuaFilename      = "test.lua"; // script that gets executed
	...
	pData->nIndex              = 3; // m_nQuestHelperID (must be > 0)
	pData->bMessageType        = 2;
	pData->bLevel              = 1;
	pData->nExp                = 0;
	...
	// index into m_QuestMonsterArray, but is also the index into m_questMap
	pData->sEventDataIndex     = 30; // checked before running (sEventDataIndex == 0; depth of quest? - 0 is entry point?)
	pData->bEventStatus        = 1; // checked before running (the start of event 30)

	pData->nEventTriggerIndex  = 35025; // nEventID
	pData->nEventCompleteIndex = 35030;
	pData->nExchangeIndex      = 0;
	pData->nEventTalkIndex     = 0;
	pData->strLuaFilename      = "test.lua"; // script that gets executed
	...
	pData->nIndex              = 4; // m_nQuestHelperID (must be > 0)
	pData->bMessageType        = 2;
	pData->bLevel              = 1;
	pData->nExp                = 0;
	...
	pData->sEventDataIndex     = 30; // checked before running (sEventDataIndex == 0; depth of quest? - 0 is entry point?)
	pData->bEventStatus        = 2; // checked before running (the start of event 30)

	pData->nEventTriggerIndex  = 35022; // nEventID
	pData->nEventCompleteIndex = 0;
	pData->nExchangeIndex      = 0;
	pData->nEventTalkIndex     = 0;
	pData->strLuaFilename      = "test.lua"; // script that gets executed
