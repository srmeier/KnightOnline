// UILogInOld.cpp: implementation of the CUILogInOld class.
//
//////////////////////////////////////////////////////////////////////
...
bool CUILogInOld::ReceiveMessage(CN3UIBase* pSender, uint32_t dwMsg)
{
	...
	if (dwMsg == UIMSG_BUTTON_CLICK)
	{
		...
		{
			CGameProcedure::s_pProcLogInOld->MsgSend_AccountLogIn(LIC_KNIGHTONLINE);
			return true;
		}
		else if (pSender == m_pBtn_Connect)
		{
			CGameProcedure::s_pProcLogInOld->ConnectToGameServer(); // 고른 게임 서버에 접속
			return true;
		}
		...
		{
			if (!CGameProcedure::s_pProcLogInOld->m_szRegistrationSite.empty())
			{
				ShellExecute(NULL, "open", CGameProcedure::s_pProcLogInOld->m_szRegistrationSite.c_str(), NULL, NULL, SW_SHOWNORMAL);
			}
...
bool CUILogInOld::Load(HANDLE hFile)
{
...
void CUILogInOld::AccountIDGet(std::string& szID)
{
	if (m_pEdit_id)
...
void CUILogInOld::AccountPWGet(std::string& szPW)
{
	if (m_pEdit_pw)
...
void CUILogInOld::ConnectButtonSetEnable(bool bEnable)
{
...
void CUILogInOld::FocusToID()
{
	if (m_pEdit_id) m_pEdit_id->SetFocus();
}

void CUILogInOld::FocusCircular()
{
...
void CUILogInOld::InitEditControls()
{
	if (m_pEdit_id)
	{
...
bool CUILogInOld::ServerInfoAdd(const __GameServerInfoOld& GSI)
{
	m_ListServerInfos.push_back(GSI);
	return true;
}

bool CUILogInOld::ServerInfoGet(size_t iIndex, __GameServerInfoOld& GSI)
{
...
bool CUILogInOld::ServerInfoGetCur(__GameServerInfoOld& GSI)
{
	GSI.Init();
...
void CUILogInOld::ServerInfoUpdate()
{
	...
	{
		//sort(m_ListServerInfos.begin(), m_ListServerInfos.end(), not2(__GameServerInfoOld()));
...
void CUILogInOld::OpenServerList()
{
...
void CUILogInOld::SetVisibleLogInUIs(bool bEnable)
{
...
void CUILogInOld::RecalcGradePos()
{
...
bool CUILogInOld::OnKeyPress(int iKey)
{
	if (!m_bLogIn)
	{
		switch (iKey)
