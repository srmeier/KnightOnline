...
public class RoxyFilemanHandler : IHttpHandler, System.Web.SessionState.IRequiresSessionState
{
    ...
    protected string GetFilesRoot()
    {
        ...
        string path = GetSetting("FILES_ROOT");
        ...
        if (ret == "")
            ...
            ret = _context.Server.MapPath("~/Content/Upload");

        else
            ret = FixPath(ret);
        if (path == "")
            path = "~/Content/Upload";
        string pat = _context.Server.MapPath(path);
        if (!Directory.Exists(pat))
            Directory.CreateDirectory(pat);
        if (path == "~/Content/Upload" || path == "/Content/Upload")
        {
            if (!Directory.Exists(pat + "\\Dosya"))
                Directory.CreateDirectory(pat + "\\Dosya");
            if (!Directory.Exists(pat + "\\Resimler"))
                Directory.CreateDirectory(pat + "\\Resimler");
            if (!Directory.Exists(pat + "\\Medya"))
                Directory.CreateDirectory(pat + "\\Medya" );
    ...
    protected void CheckPath(string path)
    {
        ...
        {
            throw new Exception("Yetki Erişimi Yok " + path);
        }
    }
    ...
    protected void ListDirTree(string type)
    {
        ...
        if (!d.Exists)
            throw new Exception("Hatali dizin dosyasi. Ayarlari kontrol ediniz.");
        string pat = d.FullName + "\\Resimler";
        if (type == "file")
        {
            pat = d.FullName + "\\Dosya";
        }
        else if (type == "media")
        {
            pat = d.FullName + "\\Medya";

        }
        ArrayList dirs = ListDirs(pat);
        dirs.Insert(0, pat);
            ...
            for (int i = 0; i < HttpContext.Current.Request.Files.Count; i++)
            {
                if (CanHandleFile(HttpContext.Current.Request.Files[i].FileName))
                {
                    ...
                    if (GetFileType(new FileInfo(filename).Extension) == "image")
                    {
                        ...
                        WebImage resim = new WebImage(dest);
                        resim.Resize(w, h, true).Crop(1,1);
                      //  resim.AddTextWatermark("Ilk", fontColor: "White", fontSize: 20).Save();
                       // ImageResize(dest, dest, w, h);

                        if (!Directory.Exists(path + "\\thumbnail"))
                            Directory.CreateDirectory(path + "\\thumbnail");
                        resim.Resize(100, 100, true).Crop(1, 1).Save(path + "\\thumbnail\\" + filename);
